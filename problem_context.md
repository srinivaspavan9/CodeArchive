...
| `problems/convex_hull_trick_max_score_stone_jumps.py` | Jumping-stones max-score problem. Given a sequence of non-negative stone values, jump from index 0 to the end; a jump i→j scores stones[j]·(j−i), and you want to maximize the total. SOLVED with two DP approaches (top-down memoized + bottom-up), both O(n²) — the user's "O(n)" note was wrong. The genuine optimum is the convex hull trick / Li Chao tree at O(n log V), added by Claude in an AI-fenced block, since dp[j] = j·stones[j] + max over lines f_i(x)=−i·x+dp[i] at x=stones[j]. |
