...
| `problems/unsolved_number_of_islands_in_tree.py` | Google phone-screen question: count the "islands" of 1s in a TREE whose nodes hold 0/1 values (an island = a maximal connected group of 1-valued nodes via parent/child edges). Tree (not grid) version of number-of-islands; interviewer wanted O(n) time, O(1) extra space. UNSOLVED — question only, no code yet; expected technique is a tree DFS/traversal counting 1-roots whose parent isn't a 1. |
